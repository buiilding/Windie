...
function reduceConversationRuntimeState(state, event) {
    ...
    if (event.type === 'tool_call') {
        const key = (0, toolCorrelationIds_js_1.resolveToolWaitId)(event.payload);
        return {
            ...base,
            phase: 'tool_call_pending',
            pendingTools: key
                ? { ...state.pendingTools, [key]: event.payload }
                : state.pendingTools,
        };
    }
