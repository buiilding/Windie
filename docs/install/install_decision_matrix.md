...
read_when:
  - When deciding whether a task needs source setup, packaged install, local reinstall, custom hosted deployment endpoint configuration, or release packaging validation.
...
| route app to a non-default backend | endpoint override | `BACKEND_HTTP_URL`, `BACKEND_WS_URL` | desktop/local-runtime traffic reaches intended backend | Private hosted service health by itself |
...
| --- | --- |
| backend-only API/provider/agent change | private backend tests plus route/provider-specific checks |
