  ...
  const renderContent = () => {
    if (type === 'json' || type === 'system-prompt') {
      try {
        ...
        return <pre className="transparency-content-text">{content}</pre>;
      }
    }
    if (type === 'xml') {
      // For XML content (like full messages with system_context, memory tags, etc.)
      // Preserve formatting and show as preformatted text
      return <pre className="transparency-content-text" style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{content}</pre>;
    }
    return <pre className="transparency-content-text">{content}</pre>;
  };
